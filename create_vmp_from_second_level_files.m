...
for i = 1:length(matfilprefixs)
    ...
    sigfdr = fdr_bh(pval,0.01,'pdep','yes');
    % compute neighbours: 
    ...
    % real data 
    tosavevmp.Map(1) = rawvmp.Map(1);
    tosavevmp.Map(1).VMPData = double(dataFromAnsMatBackIn3d);
    tosavevmp.Map(1).Name = fn;
    % voxel wise fdr 
    dataFromAnsMatBackIn3d = scoringToMatrix(mask,sigfdrplot',locations);
    tosavevmp.Map(2) = rawvmp.Map(1);
    tosavevmp.Map(2).VMPData = dataFromAnsMatBackIn3d;
    tosavevmp.Map(2).Name = sprintf('sig fdr voxel wise .01 (%d)',sum(sigfdr(:)));
    % fwer control 
    [sigfwer,sigbonf, clustdata] = calcFWERcontrol(avgAnsMat,mask,locations);
    dataFromAnsMatBackIn3d = scoringToMatrix(mask,sigfwer,locations);
    tosavevmp.Map(3) = rawvmp.Map(1);
    tosavevmp.Map(3).VMPData = dataFromAnsMatBackIn3d;
    tosavevmp.Map(3).Name = sprintf('sig FWER cluster wise .05 (%d)',sum(sigfwer(:)));
